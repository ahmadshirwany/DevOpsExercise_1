...
def get_uptime():
    ...
    return f"Timestamp1 Uptime: {uptime_h} hours, Free Memory: {freespace_in_mb} MB, Time: {time}"
...
def log():
    try:
        response = requests.get("http://storage:8003/log")
        return Response(content=response.text, media_type="text/plain")
    except requests.exceptions.RequestException as e:
        return Response(content=f"Error retrieving logs from storage: {e}", media_type="text/plain")
